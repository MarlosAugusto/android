...
package com.amperbackup.android.sso;
...
import com.amperbackup.android.sso.aidl.ParcelFileDescriptorUtil;
import com.amperbackup.android.sso.aidl.IInputStreamService;
import com.amperbackup.android.sso.aidl.NextcloudRequest;
import com.amperbackup.android.sso.aidl.ParcelFileDescriptorUtil;
...
import static com.amperbackup.android.sso.Constants.EXCEPTION_HTTP_REQUEST_FAILED;
import static com.amperbackup.android.sso.Constants.EXCEPTION_INVALID_REQUEST_URL;
import static com.amperbackup.android.sso.Constants.EXCEPTION_INVALID_TOKEN;
import static com.amperbackup.android.sso.Constants.EXCEPTION_UNSUPPORTED_METHOD;
...
public class InputStreamBinder extends IInputStreamService.Stub {
    ...
    private InputStream processRequest(final NextcloudRequest request, final InputStream requestBodyInputStream)
        ...
        if (request.getUrl().length() == 0 || request.getUrl().charAt(0) != PATH_SEPARATOR) {
            throw new IllegalStateException(Constants.EXCEPTION_INVALID_REQUEST_URL,
        ...
        if (status >= HTTP_STATUS_CODE_OK && status < HTTP_STATUS_CODE_MULTIPLE_CHOICES) {
            ...
            throw new IllegalStateException(Constants.EXCEPTION_HTTP_REQUEST_FAILED,
    ...
    private boolean isValid(NextcloudRequest request) {
        ...
        SharedPreferences sharedPreferences = context.getSharedPreferences(Constants.SSO_SHARED_PREFERENCE,
                                                                           Context.MODE_PRIVATE);
        String hash = sharedPreferences.getString(callingPackageName + Constants.DELIMITER + request.getAccountName(), "");
