 ...
 */
package com.amperbackup.client.onboarding
...
internal class OnboardingServiceImpl constructor(
    ...
    private val accountProvider: com.amperbackup.client.account.CurrentAccountProvider
) : OnboardingService {
    ...
    override fun launchActivityIfNeeded(activity: Activity) {
        if (!resources.getBoolean(R.bool.show_whats_new) || activity is com.amperbackup.client.onboarding.WhatsNewActivity) {
            return
        }

        if (shouldShowWhatsNew(activity)) {
            activity.startActivity(Intent(activity, com.amperbackup.client.onboarding.WhatsNewActivity::class.java))
        }
    }
    ...
    override fun launchFirstRunIfNeeded(activity: Activity): Boolean {
        ...
        if (canLaunch) {
            val intent = Intent(activity, com.amperbackup.client.onboarding.FirstRunActivity::class.java)
